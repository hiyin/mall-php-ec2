	...
	color: function (color, elem, prop) {
		...
		if (gradName) {
			...
			if (gradients[key]) {
				...
				// The gradient needs to keep a newslist of stops to be able to destroy them
...
Highcharts.VMLElement = VMLElement = {
	...
	clip: function (clipRect) {
		...
		if (clipRect) {
			...
			erase(clipMembers, wrapper); // Ensure unique newslist of elements (#1258)
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the newslist item text and add it to the group
